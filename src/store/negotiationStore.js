...
// Mock scenario data (in a real app, this would come from an API)
const mockScenario = {
  rounds: [
    {
  ...
  tips: {
    "I": { videoUrl: "/videos/tip_I.mp4" },
    "E": { videoUrl: "/videos/tip_E.mp4" },
    "N": { videoUrl: "/videos/tip_N.mp4" },
    "Debrief": { videoUrl: "/videos/tip_debrief.mp4" }
  }
};
...
const useNegotiationStore = create((set, get) => ({
  ...
  loadScenario: async () => {
    ...
    set({ scenario: mockScenario });
  },
  
  advanceRound: () => {
    const { currentRound, scenario } = get();
